...
class Layer extends React.Component {
    ...
    cell = (x, y, on) => {
        ...
        const click = () => this.click(x, y, matrix, rx, ry);
        const mouseMove = (e) => {
            if (e.buttons) {
                if (this.lastMove[0] !== x || this.lastMove[1] !== y) {
                    this.click(x, y, matrix, rx, ry);
                    this.lastMove = [x, y];
                }
            } else {
                this.lastMove = [-1, -1];
        ...
        return <div style={{'user-select': 'none', 'background-color': on ? this.state.color : ''}} key={`${x}:${y}`}
                    className={on ? 'on' : 'off'}
                    onClick={click} onMouseMove={mouseMove}>&nbsp;</div>
    };
...
0x${this.state.row3}`;
        ...
        return <div className="App">
            ...
            <div>
            <textarea style={{height: '80px'}} value={value} onClick={this.handleClick}
                      onChange={this.handleChange}></textarea>
            </div>
            <div>
                <input type="color" value={this.state.color} onChange={this.newColor}></input>
            </div>
        </div>
    }
}
